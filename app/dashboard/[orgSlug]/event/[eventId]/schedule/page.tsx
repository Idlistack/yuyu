...
export default async function EventScheduleManagePage({ params }: { params: Promise<{ orgSlug: string; eventId: string }> }) {
  ...
  const event = await prisma.event.findFirst({ where: { id: eventId, organisationId: access.organisation.id }, include: { sessions: { include: { speakers: { include: { speaker: { select: { id: true, name: true } } } } }, orderBy: [{ startDateTime: "asc" }, { sortOrder: "asc" }] }, speakers: { select: { id: true, name: true }, orderBy: [{ sortOrder: "asc" }, { name: "asc" }] }, venue: { include: { rooms: { orderBy: { sortOrder: "asc" } } } } } });
  if (!event) notFound();
