...
export default async function SessionPage({ params }: { params: Promise<{ orgSlug: string; eventSlug: string; sessionSlug: string }> }) {
  ...
  const [session, programme] = await Promise.all([
    prisma.eventSession.findFirst({ where: { eventId: event.id, slug: sessionSlug, visibility: ContentVisibility.PUBLISHED }, include: { speakers: { include: { speaker: true }, orderBy: { sortOrder: "asc" } }, resources: { where: { visibility: ContentVisibility.PUBLISHED } } } }),
