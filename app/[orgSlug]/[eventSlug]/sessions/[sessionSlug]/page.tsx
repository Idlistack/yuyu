...
export default async function SessionPage({ params }: { params: Promise<{ orgSlug: string; eventSlug: string; sessionSlug: string }> }) {
  ...
  const [session, programme] = await Promise.all([
    prisma.eventSession.findFirst({ where: { eventId: event.id, slug: sessionSlug, visibility: ContentVisibility.PUBLISHED }, include: { track: true, speakers: { include: { speaker: true }, orderBy: { sortOrder: "asc" } }, resources: { where: { visibility: ContentVisibility.PUBLISHED } } } }),
    prisma.eventSession.findMany({ where: { eventId: event.id, visibility: ContentVisibility.PUBLISHED }, select: { id: true, trackId: true, startDateTime: true, endDateTime: true, sortOrder: true, delayMinutes: true } }),
  ]);
  if (!session) notFound();
  const effective = effectiveEventProgram(programme).find((item) => item.id === session.id);
  if (!effective) notFound();
  return <Stack spacing={2} sx={{ maxWidth: 800, py: 4 }}><Typography component="a" href={`/${org.slug}/${event.slug}`} color="primary">{event.title}</Typography><Typography variant="h3">{session.title}</Typography><Typography color="text.secondary">{effective.effectiveStartDateTime.toLocaleString(undefined, { timeZone: event.timezone })} · {session.track.name}</Typography><Typography component="div" dangerouslySetInnerHTML={{ __html: sanitizeRichText(session.descriptionHtml) }} />{session.speakers.length ? <><Typography variant="h5">Speakers</Typography>{session.speakers.filter((item) => item.speaker.visibility === ContentVisibility.PUBLISHED).map(({ speaker }) => <Typography key={speaker.id} component="a" href={`/${org.slug}/${event.slug}/speakers/${speaker.slug}`}>{speaker.name}</Typography>)}</> : null}</Stack>;
}
