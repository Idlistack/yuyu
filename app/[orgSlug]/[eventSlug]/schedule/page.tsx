...
export default async function EventSchedulePage({ params }: { params: Promise<{ orgSlug: string; eventSlug: string }> }) {
  ...
  if (!org) notFound();
  const event = await prisma.event.findUnique({ where: { organisationId_slug: { organisationId: org.id, slug: eventSlug } }, include: { page: { select: { isPublished: true } }, sessions: { where: { visibility: ContentVisibility.PUBLISHED }, orderBy: [{ startDateTime: "asc" }, { sortOrder: "asc" }] } } });
  if (!event) notFound();
  ...
  return <Stack spacing={2} sx={{ maxWidth: 800, py: 2 }}><Typography variant="overline">{org.name}</Typography><Typography variant="h3">{event.title} program</Typography>{sessions.map((session) => <Stack key={session.id} component="a" href={`/${org.slug}/${event.slug}/sessions/${session.slug}`} sx={{ p: 2, color: "text.primary", textDecoration: "none", border: 1, borderColor: "divider", borderRadius: 2 }}><Typography sx={{ fontWeight: 700 }}>{session.title}</Typography><Typography variant="body2">{session.effectiveStartDateTime.toLocaleString(undefined, { timeZone: event.timezone })} · {session.location ?? "Location TBA"}</Typography></Stack>)}</Stack>;
}
